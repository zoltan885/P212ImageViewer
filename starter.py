...
#        pg.setConfigOption('useNumba', True)  # probably too new!
    ...
    def prepare(self):
        ...
        self.pushButton.clicked.connect(self.qttransformtest)
    ...
    def loadWithClass(self):
        print('loadWithClass function called')
        self.imageDataSet.dataFolderLoad.signals.started.connect(self.addProgressBar)
        self.imageDataSet.dataFolderLoad.signals.progress.connect(self.updateProgressBar)
        self.imageDataSet.dataFolderLoad.signals.finished.connect(self.removeProgressBar)
        self.imageDataSet.signals.chunkLoaded.connect(self._intermediateLoadState)
        self.imageDataSet.loadFolder()
...
#######################################
    ...
    def addLineCut(self):
        ...
        x0,y0 = (view[0][1]+2*view[0][0])/3, (view[1][1]+view[1][0])/2
    ...
    def addROI(self):
        ...
        n = len(self.ROIs)
        view = self.plotItem.getViewBox().state['viewRange']  # [[xmin,xmax],[ymin,ymax]]
        x0,y0 = (view[0][1]+2*view[0][0])/3, (view[1][1]+2*view[1][0])/3
    ...
    def showROI(self):
        ...
        if self.comboBox_ROI.currentText() == 'ROI':
            # disable visible image transformations
            self.comboBox_transform.setCurrentText('none')
...
###################################
    def qttransformtest(self):
        self.tr.scale(-1, 1)
        self.tr.translate(-73,0)
        self.updateImage()
